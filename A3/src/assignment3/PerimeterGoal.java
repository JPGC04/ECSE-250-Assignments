	...
	public int score(Block board) {
		...
		for (int i = 0; i < flatBoard.length; i++) {
			for (int j = 0; j < flatBoard[0].length; j++) {
				if (flatBoard[i][j] == this.targetGoal && (i == 0 || j == 0 || i == flatBoard.length - 1 || j == flatBoard[0].length - 1)) {
					if ((i == 0 && j == 0) || (i == 0 && j == flatBoard.length - 1) || (i == flatBoard.length - 1 && j == 0) || (i == flatBoard.length - 1 && j == flatBoard.length - 1)) {
						sum += 2;
					} else {
						sum +=1;
					}
